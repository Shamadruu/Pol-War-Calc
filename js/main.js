  ...
  main.nation.data.City = function() {
    ...
    this.update = function() {
      ...
      for (var x in this.buildings) {
        //Power is different from the others (i.e its consumption is variable)
        if (x == "power") {
          ...
          for (var y in this.buildings.power) {
            if (this.buildings.power[y] > 0) {
              ...
              //Check if only power source and, if so, calculate resource consumption. Due to a lack of information about how this game calculates consumption in the case of more than one power source, all will use their max consumption. (sorry)
              // if(main.nation.data.buildings.power[y].powerSupply * this.buildings.power[y] == this.powerSupply){
      ...
      //General
      this.avgIncome = ((this.commerce / 50) * (725 / (main.nation.inputData.taxRate * 1000))) + (725 / (main.nation.inputData.taxRate * 1000));
      ...
      this.pop = Math.round(this.pop - (this.disease * this.infra) - (10 * this.crime * this.infra) + 25);
      if (this.pop < 0) {
        this.pop = 0;
      }
      this.popDensity = this.pop / this.land;
      ...
      this.revenue.money.prod += this.avgIncome * this.pop * main.nation.inputData.taxRate * (1 + (main.nation.inputData.colorBonus + main.nation.inputData.treasureBonus) / 100);
...
main.display = {
  events: function() {
    ...
    $("#nationConfig").on("change", "input", function() {
      if ($(this).attr("id") == "nationalTaxRate") {
  ...
  nation: {
    genNationConfig: function() {
      //Generates the body of nationConfig
      var HTML = '<div class="row"><div class="col-sm-3 header">National Tax Rate</div><div class="col-sm-3"><input id="nationalTaxRate" name="tax" type="number" min="1" max="100" value="' + (main.nation.inputData.taxRate * 100).toFixed(2) + '">%</div><div class="col-sm-3 header">Total Income Bonus</div><div class="col-sm-3"><input id="nationBonus" name="bonus" type="number" min="0" max="100" value="' + main.nation.inputData.colorBonus + '">%</div></div><div class="row"><div class="col-sm-3 header">War? </div><div class="col-sm-3"><input type="checkbox" name="war" id="war"></div><div class="col-sm-3 header">Starving?</div><div class="col-sm-3"><input type="checkbox" name="starve" id="starve"></input></div></div>';
      return HTML;
    },
